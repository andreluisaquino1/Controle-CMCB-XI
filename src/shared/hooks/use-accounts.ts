...
import { ACCOUNT_NAME_TO_LEDGER_KEY, ENTITY_TYPE } from "@/shared/lib/constants";
...
export function useAccountsByEntityType(entityType: EntityType | null, includeInactive = false) {
  const { isDemo } = useAuth();
  const { accounts } = useDemoData();
...
export function useAssociacaoAccounts() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      const entity = entities.find(e => e.type === ENTITY_TYPE.ASSOCIACAO);
  ...
  if (isDemo) {
    const accountsData = demoAccounts.length > 0 ? demoAccounts : demoStore.getAccounts();
    const assoc = MOCK_ENTITIES.find(e => e.type === ENTITY_TYPE.ASSOCIACAO);
